...
DROP FUNCTION  if exists getCurrentCost(numeric, date, character varying, numeric, numeric);
CREATE OR REPLACE FUNCTION getCurrentCost(p_M_Product_ID numeric, p_Date date, p_AcctSchema_ID numeric,
                                          p_CostingMethod character varying,
...
WITH x as
         (
             ...
             from m_costdetail cd
                      JOIN m_costelement costElement on cd.m_costelement_id = costElement.m_costelement_id AND
                                                        costElement.costingmethod = p_CostingMethod
...
from M_Cost cost
         JOIN m_costelement costElement
              on cost.m_costelement_id = costElement.m_costelement_id AND costElement.costingmethod = p_CostingMethod
